  ...
  return (
    <div className="space-y-4">
      ...
      {selectedRegion && (
        <div className="rounded-md border border-forensic-gold/35 bg-forensic-gold/10 p-4">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <p className="font-semibold">{selectedRegion.id} · {selectedRegion.label}</p>
            ...
            <StatusBadge tone={reviewed ? 'success' : toneForStatus(selectedRegion.reviewStatus)}>
              {reviewed ? '已复核' : selectedRegion.reviewStatus}
            </StatusBadge>
          </div>
      ...
      <div className="grid grid-cols-1 gap-2">
        ...
        <Link
          to="/analysis/sample"
          className="rounded-md border border-graphite-800 bg-graphite-950 px-4 py-2 text-center text-sm font-medium text-forensic-text"
        >
          发送到分析
        </Link>
      </div>
