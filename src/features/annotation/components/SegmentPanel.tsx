  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h3 className="mt-1 text-lg font-semibold">{segment.id} · {segment.label}</h3>
      </div>
      ...
      <StatusBadge tone={reviewed ? 'success' : toneForStatus(segment.reviewStatus)}>
        {reviewed ? '已复核' : segment.reviewStatus}
      </StatusBadge>
      <div className="grid gap-2">
        <button type="button" onClick={onRun} className="rounded-md border border-forensic-gold/40 bg-forensic-gold/10 px-4 py-2 text-sm font-medium text-forensic-gold">
          {running ? '正在处理' : '运行自动标注'}
        </button>
        <button type="button" onClick={onReview} className="rounded-md border border-forensic-olive/40 bg-forensic-olive/10 px-4 py-2 text-sm font-medium text-forensic-olive">
          标记片段已复核
        </button>
        <Link to="/analysis/sample" className="rounded-md border border-graphite-800 bg-graphite-950 px-4 py-2 text-center text-sm font-medium">
          发送到分析
        </Link>
      </div>
    </div>
